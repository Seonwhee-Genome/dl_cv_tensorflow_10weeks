...
## number of filters are getting decrease
...
def encoder(batch_size, df_dim, ch, rows, cols):
    ...
    mean = Dense(z_dim, name="e_h3_lin", init=normal)(model)  ## getting values of latent variables  (slide 8)
...
def get_model(sess, image_shape=(80, 160, 3), gf_dim=64, df_dim=64, batch_size=64,
    ...
    with tf.variable_scope(name), tf.device("/gpu:{}".format(gpu)):
      # sizes
      ch = image_shape[2]
      rows = [image_shape[0]/i for i in [16, 8, 4, 2, 1]]  # for DCGAN
      ...
      print "G.shape: ", G.output_shape  # Check weights and biases
      ...
      Z2 = Input(batch_shape=(batch_size, z_dim), name='more_noise') # Zp noise distribution (Slide 15)
      Z = G.input
      Img = D.input
      G_train = G(Z)
      E_mean, E_logsigma = E(Img)
      G_dec = G(E_mean + Z2 * E_logsigma)
      D_fake, F_fake = D(G_train) # xdot, xp
      D_dec_fake, F_dec_fake = D(G_dec) # y1, yhat1
      D_legit, F_legit = D(Img) # True image x

      # costs
      recon_vs_gan = 1e-6   # gamma (Slide 16)
      like_loss = tf.reduce_mean(tf.square(F_legit - F_dec_fake)) / 2.  # likelihood loss
      kl_loss = tf.reduce_mean(-E_logsigma + .5 * (-1 + tf.exp(2. * E_logsigma) + tf.square(E_mean))) # Lprior
# discriminator loss
      d_loss_legit = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(D_legit, tf.ones_like(D_legit))) #(Slide17)
...
# generator loss
    ...
    def train_g(images, z, counter, sess=sess):
      ...
      outs = sess.run(outputs + updates, feed_dict={Img: images, Z: z, Z2: z2, K.learning_phase(): 1}) # real output
    ...
    def f_load():
      try:
        ...
        G.load_weights(checkpoint_dir+"/G_weights.keras") # like .ckpt
